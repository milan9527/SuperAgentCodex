          ...
          if (appConfig.agentRuntime === 'agentcore') {
            try {
              ...
              await agentCoreCommandService.runCommand(sessionId,
                `mkdir -p /workspace/${skillsDir}/${result.name}`
              );
              await writeSkillFiles(result.localPath, `${skillsDir}/${result.name}`);
            ...
            try {
              ...
              await uploadSkillFiles(
                result.localPath,
                `${s3Prefix}${workspaceManager.getActiveRuntimeLayout().skillsDir}/${result.name}`,
              );
