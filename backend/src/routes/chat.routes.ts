...
import type { MCPServerSDKConfig } from '../services/agent-types.js';
...
function normalizeMcpServerConfig(config: Record<string, unknown>): MCPServerSDKConfig {
  const type = config.type;
  if (type === 'stdio') {
    if (typeof config.command !== 'string' || !config.command.trim()) {
      throw AppError.validation('stdio MCP server requires a command');
    }
    return {
      type,
  ...
  if (type === 'http' || type === 'sse') {
    if (typeof config.url !== 'string' || !config.url.trim()) {
      throw AppError.validation(`${type} MCP server requires a URL`);
    }
    return {
      type,
      url: config.url,
  ...
  throw AppError.validation('MCP server type must be stdio, http, or sse');
}
...
export async function chatRoutes(fastify: FastifyInstance): Promise<void> {
   ...
   */
  fastify.get<{ Querystring: { business_scope_id?: string } }>(
    '/sessions',
    {
          ...
          200: {
            ...
            items: {
              ...
              properties: {
                ...
                claude_session_model: { type: 'string', nullable: true },
                provider_runtime: { type: 'string', nullable: true },
                provider_thread_id: { type: 'string', nullable: true },
                provider_thread_model: { type: 'string', nullable: true },
                provider_thread_metadata: { type: 'object' },
   ...
   * Interrupt the active provider turn without treating SSE disconnect as stop.
   */
  fastify.post<{ Params: { id: string } }>(
    '/sessions/:id/stop',
    {
      preHandler: [authenticate],
      schema: {
        description: 'Interrupt the active agent turn for a chat session',
        tags: ['Chat'],
        security: [{ bearerAuth: [] }],
        params: {
          type: 'object',
        ...
        response: {
          200: {
            type: 'object',
            properties: {
              status: { type: 'string', enum: ['interrupted', 'idle'] },
    ...
    async (request, reply) => {
      ...
      const isAdmin = request.user!.role === 'owner' || request.user!.role === 'admin';
      if (!isAdmin && session.user_id !== request.user!.id) {
        throw AppError.forbidden('You cannot stop another user\'s chat session');
      }

      const wasActive = await chatService.stopSession(id, request.user!.orgId);
      auditService.log({
        ...
        resourceId: id,
        scopeId: session.business_scope_id,
        metadata: {
          runtime: session.provider_runtime,
      ...
      return reply.status(200).send({
        status: wasActive ? 'interrupted' : 'idle',
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (appConfig.agentRuntime === 'agentcore') {
          ...
          // (AGENTS.md, .agents/, .codex/) — it never receives the files the agent
  ...
  fastify.delete<{ Params: { id: string; skillName: string }; Querystring: { removeFromScope?: string } }>(
    ...
    async (request, reply) => {
      ...
      if (appConfig.agentRuntime === 'agentcore') {
        try {
          const skillsDir = workspaceManager.getActiveRuntimeLayout().skillsDir;
          await agentCoreCommandService.deleteDirectory(
            session.id,
            `${skillsDir}/${request.params.skillName}`,
          );
        } catch (err) {
   ...
   * List MCP servers configured in the session workspace.
   */
  fastify.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const servers = Object.entries(mcpServers).map(([name, cfg]) => ({ name, ...cfg }));
      return reply.status(200).send({ servers });
    },
  );
   ...
   * Add or update an MCP server in the session workspace.
   */
  fastify.put<{ Params: { id: string }; Body: { name: string; config: Record<string, unknown> } }>(
    ...
    async (request, reply) => {
      ...
      await workspaceManager.updateWorkspaceMcpServer(
        request.user!.orgId,
        session.business_scope_id,
        session.id,
        request.body.name,
        normalizeMcpServerConfig(request.body.config),
      );
   ...
   * Remove an MCP server from the session workspace.
   */
