...
export class AgentNodeExecutor extends BaseNodeExecutor {
  ...
  async execute(params: NodeExecutionParams): Promise<NodeExecutionResult> {
    ...
    if (agentId && organizationId) {
      return agentStatusService.withBusyStatus(agentId, organizationId, () =>
        this.executeWithAgentRuntime(node, metadata, query, agentId, organizationId, context),
      );
    }
  ...
  /**
   * Execute using the configured agent runtime with full workspace support.
   * Agent gets access to skills, subagents, and tools.
   */
  private async executeWithAgentRuntime(
    ...
    try {
      ...
      for await (const event of conversation) {
        if (event.type === 'session_start') {
          providerThreadId = event.providerThreadId ?? event.sessionId;
          ...
          throw new Error(event.message || `${runtime.name} agent runtime error`);
        }
      }
      ...
      const runtime = await this.getAgentRuntime();
      console.error(`Agent node (${runtime.name}) failed: ${errorMessage}`, {
        nodeId: node.id,
