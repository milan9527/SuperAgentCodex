...
export class MessageRouter {
  ...
  async route(params: {
    ...
    const fallbackMember = activeMembers[0];
    if (!fallbackMember) {
      throw new Error('No active members in room');
    }
    ...
    if (activeMembers.length === 1) {
      return {
        targetAgentId: fallbackMember.agent_id,
        targetAgentName: fallbackMember.agent.display_name,
        confidence: 1.0,
  ...
  private detectContextContinuation(
    ...
    const lastAgentId = lastAiMessages.at(-1)?.agent_id;
  ...
  private async autoRoute(
    ...
    if (!fallbackMember) {
      throw new Error('No members available for routing');
    }
