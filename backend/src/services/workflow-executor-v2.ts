 ...
 *
 * Executes an entire workflow as a single configured agent-runtime session.
...
function serializePlanToMissionBrief(
  ...
  lines.push(`  Read them (in \`${workspaceManager.getActiveRuntimeLayout().skillsDir}/\`) to understand the methodology, then execute the step`);
...
export class WorkflowExecutorV2 {
  ...
  private async *executeSegment(
    ...
    const instructionsFile = workspaceManager.getActiveRuntimeLayout().instructionsFile;
    await writeFile(join(workspacePath, instructionsFile), missionBrief, 'utf-8');
    ...
    const mcpServers: Record<string, MCPServerSDKConfig> = {
      'workflow-progress': progressBridge.config,
    };
    ...
    try {
      ...
      await progressBridge.close().catch((error) => {
        console.warn('[workflow-v2] Failed to close progress bridge:', error);
      });
    }
  }
