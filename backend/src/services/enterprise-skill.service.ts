...
export class EnterpriseSkillService {
  ...
  async installToWorkspace(
    ...
    if (appConfig.agentRuntime === 'agentcore') {
      ...
      try {
        ...
        await uploadSkillFiles(skillSourceDir, `${s3Prefix}${skillsDir}/${entry.skill.name}`);
      } catch (err) {
      ...
      try {
        ...
        await agentCoreCommandService.runCommand(sessionId, `mkdir -p /workspace/${skillsDir}/${entry.skill.name}`);
        await writeSkillFiles(skillSourceDir, `${skillsDir}/${entry.skill.name}`);
      } catch (err) {
  ...
  async publishFromWorkspace(
    ...
    const skillSourceDir = join(
      workspacePath,
      workspaceManager.getActiveRuntimeLayout().skillsDir,
      options.skillName,
    );
