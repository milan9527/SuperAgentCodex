 ...
 *
 * Instead of dumping the entire workflow into one root instruction file and hoping
 * the agent self-navigates, this orchestrator walks the DAG from the backend,
 * sending one node at a time to the configured runtime session.
 *
 * Key design decisions:
 * - The orchestrator controls the execution order (topological traversal)
 * - Agent nodes delegate to the runtime session with a focused prompt
...
export class WorkflowOrchestrator {
  ...
  private async executeAgentNode(
    ...
    // Write the task as the active runtime's focused root instruction file.
    await writeFile(
      join(workspacePath, workspaceManager.getActiveRuntimeLayout().instructionsFile),
      `# Current Task: ${node.title}\n\n${prompt}\n\nComplete this task thoroughly.`,
      'utf-8',
    );

    // Run the configured agent runtime for this single node.
  ...
  private async executeConditionNode(
    ...
    if (varMatch) {
      ...
      if (!varName || !operator || expected === undefined) {
  ...
  private async provisionWorkspace(
    ...
    const scope = scopes.data.find(s => s.id === scopeId);
