...
export class ChatService {
  ...
  async stopSession(sessionId: string, organizationId: string): Promise<boolean> {
    const session = await this.getSessionById(sessionId, organizationId);
    const wasActive = this.agentRuntime.hasSession(sessionId)
      || Boolean(session.provider_thread_id && this.agentRuntime.hasSession(session.provider_thread_id));

    await this.agentRuntime.disconnectSession(sessionId);
    ...
    return chatSessionRepository.createForUser(
      {
        ...
        claude_session_model: null,
        provider_runtime: null,
        provider_thread_id: null,
        provider_thread_model: null,
      ...
      await this.iterateWithTimeout(
        ...
        (event: ConversationEvent) => {
          if (event.type === 'session_start' && event.sessionId) {
            const sessionModel = event.model
  ...
  async streamChat(
    ...
    let providerThreadModel: string | undefined;
    ...
    let mcpServers: Record<string, import('./agent-types.js').MCPServerSDKConfig> = {};
    ...
    if (useScopeFlow) {
      ...
      providerThreadModel = result.providerThreadModel;
    ...
    let effectiveMessage = options.message.trim()
      || (options.attachedImages?.length
        ? 'Please respond to the attached image or images.'
        : options.message);
    ...
    if (options.mentionAgentId && useScopeFlow) {
      ...
      if (mentionedName) {
        requestedAgentName = mentionedName;
      ...
      const conversationGenerator = this.agentRuntime.runConversation(
            {
              ...
              imagePaths: options.attachedImages,
              requestedAgentName,
            },
            agentConfig,
            skills,
          ...
          if (event.type === 'session_start' && event.sessionId) {
            ...
            if (sessionId) {
              chatSessionRepository.updateProviderThread(
                ...
                console.error('Failed to store provider thread:', err);
              });
            }
          }
  ...
  private async prepareScopeSession(
    ...
    pluginPaths: string[];
    mcpServers: Record<string, import('./agent-types.js').MCPServerSDKConfig>;
  }> {
    ...
    const agentConfig: AgentConfig = {
      ...
      model: scopeForWorkspace.settings?.modelId as string | undefined
        ?? selectedAgent?.modelConfig?.modelId as string | undefined,
      organizationId,
    ...
    agentConfig.resolvedModel = await resolveModel(organizationId, {
      ...
      agentSelection: extractSelection(selectedAgent?.modelConfig),
    });
    ...
    return {
      ...
      subAgentInfoMap,
      pluginPaths,
      mcpServers: await this.readSessionMcpServers(workspacePath),
    };
  }
   ...
   */
  private async readSessionMcpServers(workspacePath: string): Promise<Record<string, import('./agent-types.js').MCPServerSDKConfig>> {
    return this.workspaceManager.readWorkspaceMcpServers(workspacePath);
  }
  ...
  private async prepareLegacySession(
    ...
    let session: ChatSessionEntity;
    if (!sessionId) {
      session = await this.createSession({ context: options.context ?? {} }, organizationId, userId);
      sessionId = session.id;
    } else {
      const existingSession = await chatSessionRepository.findById(sessionId, organizationId);
      if (!existingSession) {
  ...
  private async loadReplayHistory(
    ...
    const messages = await chatMessageRepository.findBySession(organizationId, sessionId, {
      limit: 24,
    });
    return messages.reverse().flatMap(message => {
      const role = message.type === 'user'
        ? 'user'
        : message.type === 'ai' || message.type === 'agent'
          ? 'assistant'
          : null;
      if (!role) return [];
      const content = extractReplayText(message.content);
...
function normalizeRuntimeProvider(
  runtime: string,
): NonNullable<ConversationEvent['provider']> {
  switch (runtime) {
    case 'codex':
    case 'agentcore':
    case 'openclaw':
...
function extractReplayText(content: string): string {
  try {
