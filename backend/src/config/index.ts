...
const envSchema = z.object({
  ...
  // Codex app-server
  CODEX_EXECUTABLE: z.string().optional().default('codex'),
  CODEX_HOME: z.string().optional(),
  CODEX_MODEL: z.string().optional(),
  CODEX_MODEL_PROVIDER: z.string().optional(),
  CODEX_REASONING_EFFORT: z.enum(['minimal', 'low', 'medium', 'high', 'xhigh']).optional().default('high'),
  CODEX_SESSION_TIMEOUT_MS: z.string().optional().default('1800000').transform(Number),
  CODEX_RESPONSE_TIMEOUT_MS: z.string().optional().default('1200000').transform(Number),
  CODEX_REQUEST_TIMEOUT_MS: z.string().optional().default('30000').transform(Number),
  CODEX_MAX_CONCURRENT_SESSIONS: z.string().optional().default('10').transform(Number),
  CODEX_MAX_IMAGE_BYTES: z.string().optional().default('20971520').transform(Number),
  ...
  AGENT_RUNTIME: z.enum(['claude', 'codex', 'agentcore', 'openclaw', 'berriai']).optional().default('claude'),
...
export const config = {
  ...
  codex: {
    executablePath: env.CODEX_EXECUTABLE,
    home: env.CODEX_HOME,
    model: env.CODEX_MODEL,
    modelProvider: env.CODEX_MODEL_PROVIDER,
    reasoningEffort: env.CODEX_REASONING_EFFORT,
    sessionTimeoutMs: env.CODEX_SESSION_TIMEOUT_MS,
    responseTimeoutMs: env.CODEX_RESPONSE_TIMEOUT_MS,
    requestTimeoutMs: env.CODEX_REQUEST_TIMEOUT_MS,
    maxConcurrentSessions: env.CODEX_MAX_CONCURRENT_SESSIONS,
    maxImageBytes: env.CODEX_MAX_IMAGE_BYTES,
    workspaceBaseDir: env.AGENT_WORKSPACE_BASE_DIR,
  },
