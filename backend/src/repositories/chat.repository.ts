...
export interface ChatSessionEntity {
  ...
  provider_thread_model?: string | null;
  provider_thread_metadata?: Record<string, unknown> | null;
  title: string | null;
...
export type ChatSessionCreateData = Omit<
  ChatSessionEntity,
  | 'id'
  | 'organization_id'
  | 'user_id'
...
> &
  Partial<
    Pick<
      ChatSessionEntity,
      'source' | 'is_starred' | 'starred_at' | 'starred_by' | 'star_category'
    >
  >;
...
export class ChatSessionRepository extends BaseRepository<ChatSessionEntity> {
  ...
  async updateProviderThread(
    ...
    const data: Partial<ChatSessionEntity> = {
      provider_runtime: runtime,
      provider_thread_id: threadId,
    };
    if (model !== undefined) data.provider_thread_model = model;
    if (metadata !== undefined) data.provider_thread_metadata = metadata;

    // Keep the legacy Claude columns synchronized throughout the rollback window.
    if (runtime === 'claude') {
      data.claude_session_id = threadId;
      if (model !== undefined) data.claude_session_model = model;
    }
    await this.update(id, organizationId, data);
  }
