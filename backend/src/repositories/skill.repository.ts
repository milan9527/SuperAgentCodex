...
export class SkillRepository {
  ...
  async create(
    ...
    data: Pick<SkillEntity, 'name' | 'display_name' | 'hash_id' | 's3_bucket' | 's3_prefix'> &
      Partial<Omit<SkillEntity, 'id' | 'organization_id' | 'name' | 'display_name' | 'hash_id' | 's3_bucket' | 's3_prefix' | 'created_at' | 'updated_at'>>
  ): Promise<SkillEntity> {
    const createData: Prisma.skillsUncheckedCreateInput = {
      organization_id: organizationId,
      business_scope_id: data.business_scope_id ?? null,
      parent_skill_id: data.parent_skill_id ?? null,
      owner_scope_id: data.owner_scope_id ?? null,
      name: data.name,
  ...
  async update(
    ...
    const updateData: Prisma.skillsUncheckedUpdateInput = {
      ...data,
