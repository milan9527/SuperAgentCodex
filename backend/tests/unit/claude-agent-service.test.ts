...
describe('claude-agent.service', () => {
  ...
  describe('buildOptions', () => {
    ...
    it('should set systemPrompt from agent config', () => {
      ...
      expect(opts.systemPrompt).toEqual(expect.stringContaining('Be concise.'));
      expect(opts.systemPrompt).toEqual(expect.stringContaining('<security>'));
    });

    it('should still apply the platform safety prompt when the agent has no prompt', () => {
      const opts = service.buildOptions(makeAgentConfig({ systemPrompt: null }), '/workspace', [], {});
      expect(opts.systemPrompt).toEqual(expect.stringContaining('<output_discipline>'));
      expect(opts.systemPrompt).toEqual(expect.stringContaining('<security>'));
    });
  ...
  describe('session timeout and cleanup', () => {
    ...
    it('should track last activity when a session starts', async () => {
      ...
      // Completed provider sessions remain trackable for bounded resume.
      expect(service.getLastActivity('sess-activity')).toBeDefined();
    });
