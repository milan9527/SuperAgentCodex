...
function mockSuccessfulSkillDownloads(manager: WorkspaceManager) {
  return vi.spyOn(manager, 'downloadSkill').mockImplementation(async (skill, targetDir) => {
    const skillDir = join(targetDir, skill.name);
    await mkdir(skillDir, { recursive: true });
    await writeFile(join(skillDir, 'SKILL.md'), `# ${skill.name}`, 'utf-8');
    return true;
  });
}
...
describe('WorkspaceManager', () => {
  ...
  describe('ensureWorkspace', () => {
    ...
    it('should reuse workspace when skills have not changed', async () => {
      ...
      const downloadSkill = mockSuccessfulSkillDownloads(manager);
      const agentId = 'agent-reuse-test';
      const skills = [createTestSkill({ id: 'skill-1', hashId: 'hash-1' })];

      // First call creates workspace
      await manager.ensureWorkspace(agentId, skills);
      const firstCallCount = downloadSkill.mock.calls.length;

      // Second call with same skills should reuse
      await manager.ensureWorkspace(agentId, skills);
      const secondCallCount = downloadSkill.mock.calls.length;
