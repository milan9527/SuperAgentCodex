...
export class CodexAppServerClient implements CodexAppServerTransport {
  private readonly options: Required<
    Pick<CodexAppServerClientOptions, 'executablePath' | 'requestTimeoutMs'>
  ...
  async start(): Promise<void> {
    ...
    await this.request('initialize', {
      clientInfo: {
        name: 'super-agent-agentcore',
        title: 'Super Agent AgentCore Runtime',
        version: '1.0.0',
      },
      capabilities: {
    ...
    if (record.id !== undefined && ('result' in record || 'error' in record)) {
      ...
      if (response.error) {
        pending.reject(new Error(
          `Codex app-server RPC error ${String(response.error.code ?? '')}: `
