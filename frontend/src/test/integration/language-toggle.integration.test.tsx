...
import { Dashboard, Chat, WorkflowEditor, Agents } from '@/pages'
...
describe('Language Toggle Persistence Integration Tests', () => {
  ...
  it('should toggle language and persist preference', async () => {
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith('super-agent-language', 'cn')

    // Toggle back to English
    const adminMenuButton2 = screen.getByRole('button', { name: /管理菜单/i })
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith('super-agent-language', 'en')
  })
  ...
  it('should translate dynamic content when language changes', async () => {
    ...
    // Wait for page to load and check for a stable page action.
    await waitFor(() => {
    ...
    // Verify page actions are translated.
    await waitFor(() => {
      expect(screen.getByRole('button', { name: /创建智能体/i })).toBeInTheDocument()
    })
