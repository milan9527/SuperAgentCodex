...
export function useKnowledge(): UseKnowledgeReturn {
  ...
  const createKnowledgeBase = useCallback(async (config: KnowledgeBaseConfig) => {
    setError(null)
    try {
      if (!KnowledgeService.createKnowledgeBase) {
        throw new Error('Knowledge base creation is not supported')
      }
  ...
  const syncAll = useCallback(async () => {
    setError(null)
    try {
      if (!KnowledgeService.syncAll) {
        throw new Error('Knowledge synchronization is not supported')
      }
