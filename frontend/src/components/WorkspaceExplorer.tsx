...
/** Check if a node is an agent definition file under a runtime config folder. */
function isAgentFile(node: FileNode): boolean {
  if (node.type !== 'file') return false
  const parts = node.path.split('/')
  // Match patterns like .codex/agents/foo.toml or legacy .claude/agents/foo.md.
  const agentsIdx = parts.lastIndexOf('agents')
  if (agentsIdx < 0) return false
  const parent = parts[agentsIdx - 1]
  if (!parent || !parent.startsWith('.')) return false
  return node.name.endsWith('.md') || node.name.endsWith('.toml')
}
...
/** Friendly display name for runtime agent definitions. */
function displayName(node: FileNode): string {
  if (isAgentFile(node)) return node.name.replace(/\.(md|toml)$/, '')
  return node.name
}
